...
def save_model(save_path, netG, netD, optG, optD, epoch, spec, config, netD_spec=None, optD_spec=None, name=None):
    ...
    if name:
        ...
        if optD is not None:
            torch.save(optD, save_path + name + "optD.pt")
        if spec:
            if netD_spec is not None:
                torch.save(netD_spec, save_path + name + "netD_spec.pt")
            if optD_spec is not None:
                torch.save(optD_spec, save_path + name + "optD_spec.pt")
        return save_path + name + "netG.pt"
...
def iteration_logs(netD, netG, optG, optD, steps, epoch, config, best_l1, best_SDR, best_reconstruct, aud, costs,
                   best_g, netD_spec=None, optD_spec=None):
    ######################
    # Update tensorboard #
    ######################
    if epoch == 0:
    ...
    # Prepare state dicts safely
    netD_state = netD.state_dict() if netD is not None else None

    if costs[-1][5] < best_l1 and not config.disable_save:
        best_g[0] = save_model(config.model_save_dir, netG.state_dict(),
                               netD_state, optG, optD, epoch, spec=True,
                               config=config, name="best_l1_")
        best_l1 = costs[-1][5]
    if costs[-1][3] < best_reconstruct and not config.disable_save:
        best_g[1] = save_model(config.model_save_dir, netG.state_dict(),
