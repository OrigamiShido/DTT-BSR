...
# IMPORT THE NEW GROUP 2 RUN EPOCH LOGIC
import utils.Runepoch_group2 as rp
...
def train(yaml_file=None):
    ...
    if not exp_file_path:
        raise Exception('Must have an experiment definition `--exp-def`!')
    ...
    # Initialize W&B
    wandb.init(config=params, project="MSG-Group2-Restoration")
    config = wandb.config
    ...
    # === Discriminator Init (Conditional) ===
    # Check if use_discriminator is in config, default to True if not present (backward compatibility)
    use_discriminator = getattr(config, 'use_discriminator', True)
    ...
    if use_discriminator:
        print("Initializing Discriminator (GAN Mode)...")
        ...
        print("Discriminator Disabled: Running in Pure Regression/Restoration Mode for Group 2.")
    ...
    if start_epoch > 0:
        ...
        optG.load_state_dict(torch.load(config.model_load_dir + str(start_epoch - 1) + "optG.pt").state_dict())

        if use_discriminator:
            netD.load_state_dict(torch.load(config.model_load_dir + str(start_epoch - 1) + "netD.pt"))
            optD.load_state_dict(torch.load(config.model_load_dir + str(start_epoch - 1) + "optD.pt").state_dict())

    # AutoBalancer (Only needed if using GAN, but we initialize it anyway to avoid errors if referenced)
    if config.adv_only:
    ...
    if config.multi_disc and use_discriminator:
        netD_spec.train()

    steps = 0
    best_SDR = -100  # Initialize lower
    ...
    for epoch in range(start_epoch, config.n_epochs):
        ...
        if (epoch + 1) % config.checkpoint_interval == 0 and epoch != start_epoch and not config.disable_save:
            # Modified save logic to handle None netD
            save_netD_state = netD.state_dict() if netD else None
            save_optD_state = optD if optD else None

            sal.save_model(config.model_save_dir, netG.state_dict(),
                           save_netD_state, optG, save_optD_state, epoch, spec=False, config=config)

        # Run Epoch
        steps, _, _ = rp.runEpoch(train_loader, config, netG, netD, optG, optD, device, epoch, steps, writer,
                                  gen_autoclip, disc_autoclip, adv_autobalancer)

        if epoch % config.validation_epoch == 0:
            with torch.no_grad():
                # Run Validation
                _, costs, aud = rp.runEpoch(valid_loader, config, netG, netD, optG, optD, device, epoch, steps, writer,
                                            gen_autoclip, disc_autoclip, adv_autobalancer, validation=True)

            # Log metrics and save best models
            best_g, best_l1, best_reconstruct, best_SDR = sal.iteration_logs(netD, netG, optG, optD, steps, epoch,
                                                                             config, best_l1, best_SDR,
                                                                             best_reconstruct, aud, costs, best_g)
